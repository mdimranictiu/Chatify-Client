...
export default function ResetPasswordPage() {
  ...
  const handleVerifyOTP = async(e) => {
    ...
    console.log(otp)
    try {
      const response= await axios.post('http://localhost:5000/reset-password/verify-otp',{email,otp})
      setSuccessMessage(response?.data?.message);
      setOtpVerified(true)
      setLoading(false)
    } catch (error) {
 ...
 const onSubmit= async(data)=>{
  ...
  try {
    const response = await axios.post('http://localhost:5000/auth/password/reset',{
